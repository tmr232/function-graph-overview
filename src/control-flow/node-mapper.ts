...
function* parentsUpTo(syntax: Parser.SyntaxNode, stopBefore: Parser.SyntaxNode): Generator<Parser.SyntaxNode> {
    let current = syntax.parent
    if (stopBefore.startIndex > syntax.startIndex || stopBefore.endIndex < syntax.endIndex) {
...
export class NodeMapper {
    ...
    private propagateInside(container: Parser.SyntaxNode): Map<Parser.SyntaxNode, string> {
        ...
        for (const [syntax, node] of this.syntaxToNode.entries()) {
            for (const parent of parentsUpTo(syntax, container)) {
                ...
                if (this.syntaxToNode.has(parent)) break;
    ...
    public getMapping(functionSyntax: Parser.SyntaxNode): Map<number, string> {
        return new Map([...this.propagateInside(functionSyntax).entries()].map(([syntax, node]) => [syntax.id, node]))
    }
}